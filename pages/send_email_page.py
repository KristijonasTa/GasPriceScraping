...
class SendEmailPage(BasePage, BaseClass):
    ...
    def extract_data_from_excel_to_json(excel_file_path, json_file_path, logger):
        try:
            logger.info(f"Extract started in excel file to create JSON file")
            excel_data = pandas.read_excel(excel_file_path, sheet_name=None)
            all_sheet_data = []
            for sheet_name, df in excel_data.items():
                sheet_data = df.head(5)
            ...
            logger.info(f"Extract ended new JSON file created with 5 lowest gast prices")
    ...
    def create_email_content(json_file_path, logger):
        try:
            logger.info(f"Taking information from JSON to create email content")
    ...
    def open_gmail(self):
        try:
            self.logger.info(f"Initializing new window")
            self.open_new_window()
            self.switch_to_new_window()
            self.get_url(TestData.url_gmail)
            self.logger.info(f"New window opened GMAIL main page loaded")
        except Exception as e:
            self.logger.error(f"Open GMAIL in new window failed: {str(e)}")

    def login_to_gmail(self):
        try:
            self.logger.info(f"Start to login to GMAIL")
    ...
    def create_new_mail_insert_recipient(self, email=None):
        try:
            ...
            self.logger.info(f"EMAIL created, recipient added")
        except Exception as e:
    ...
    def insert_subject_and_body(self):
        try:
            ...
            self.logger.info(f"Subject added, email, content added")
        except Exception as e:
            self.logger.error(f"Create new email failed: {str(e)}")
